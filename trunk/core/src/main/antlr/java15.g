 ...
 *
 *		*  "finally" clause was a root...made it a child of "try"
 *		*  Added stuff for asserts too for Java 1.4, but *commented out*
 ...
 *	  Notes:
 *	  * We only allow the "extends" keyword and not the "implements"
 *		keyword, since thats what JSR14 seems to imply.
 *	  * Thanks to Monty Zukowski for his help on the antlr-interest
 *		mail list.
 *	  * Thanks to Alan Eliasen for testing the grammar over his
 *		Fink source base
 *
 * Version 1.22 (July, 2004)
 *	  Changes by Michael Studman to support Java 1.5 language extensions
 *	  Notes:
 *	  * Added support for annotations types
 *	  * Finished off Matt Quail's generics enhancements to support bound type arguments
 *	  * Added support for new for statement syntax
 *	  * Added support for static import syntax
 *	  * Added support for enum types
 *	  * Tested against JDK 1.5 source base and source base of jdigraph project
 *	  * Thanks to Matt Quail for doing the hard part by doing most of the generics work
 *
 * Version 1.22.1 (July 28, 2004)
 *	  Bug/omission fixes for Java 1.5 language support
 *	  * Fixed tree structure bug with classOrInterface - thanks to Pieter Vangorpto for
 *		spotting this
 *	  * Fixed bug where incorrect handling of SR and BSR tokens would cause type
 *		parameters to be recognised as type arguments.
 *	  * Enabled type parameters on constructors, annotations on enum constants
 *		and package definitions
 *	  * Fixed problems when parsing if ((char.class.equals(c))) {} - solution by Matt Quail at Cenqua
 *
 * Version 1.22.2 (July 28, 2004)
 *	  Slight refactoring of Java 1.5 language support
 *	  * Refactored for/"foreach" productions so that original literal "for" literal
 *	    is still used but the for sub-clauses vary by token type
 *	  * Fixed bug where type parameter was not included in generic constructor's branch of AST
 *
 * Version 1.22.3 (August 26, 2004)
 *	  Bug fixes as identified by Michael Stahl; clean up of tabs/spaces
 *        and other refactorings
 *	  * Fixed typeParameters omission in identPrimary and newStatement
 *	  * Replaced GT reconcilliation code with simple semantic predicate
 *	  * Adapted enum/assert keyword checking support from Michael Stahl's java15 grammar
 *	  * Refactored typeDefinition production and field productions to reduce duplication
 *
 * Version 1.22.4 (October 21, 2004)
 *    Small bux fixes
 *    * Added typeArguments to explicitConstructorInvocation, e.g. new <String>MyParameterised()
 *    * Added typeArguments to postfixExpression productions for anonymous inner class super
 *      constructor invocation, e.g. new Outer().<String>super()
 *    * Fixed bug in array declarations identified by Geoff Roy
 *
 * Version 1.22.5 (January 03, 2005)
 *    Small change to tree structure
 *    * Flattened classOrInterfaceType tree so IDENT no longer has children. TYPE_ARGUMENTS are now
